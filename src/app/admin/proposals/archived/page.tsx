...
export default function AdminArchivedProposalsPage() {
  ...
  const [proposalToUnarchive, setProposalToUnarchive] = useState<{ id: string; isArchived: boolean } | null>(null);
  ...
  const handleUnarchiveClick = (proposalId: string, isArchived: boolean) => {
    setProposalToUnarchive({ id: proposalId, isArchived });
    setShowConfirmDialog(true);
  };

  const confirmUnarchiveAction = async () => {
    if (!proposalToUnarchive) return;

    const { id, isArchived } = proposalToUnarchive;
    ...
    try {
      ...
      setProposalToUnarchive(null); // Clear the state
    }
  };
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onSelect={() => handleUnarchiveClick(proposal._id, false)} className="text-green-600 focus:text-green-600">
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to {proposalToUnarchive?.isArchived ? 'archive' : 'unarchive'} this proposal?
          ...
          <DialogFooter>
            ...
            <Button variant="default" className="bg-green-600 text-white hover:bg-green-700" onClick={confirmUnarchiveAction}>
              {proposalToUnarchive?.isArchived ? 'Archive' : 'Unarchive'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
